...
3. **Zugangscodes anlegen:** `config.sample.php` nach **`config.php`** kopieren und ausfüllen:
   ```php
   'reviewers' => [
     'a1b2c3d4e5f6a7b8' => ['name' => 'Praxis Leitung', 'admin' => true],
     '9f8e7d6c5b4a3210' => ['name' => 'Physio Müller',  'admin' => false],
   ],
   ```
   Codes zufällig erzeugen: `openssl rand -hex 8`. Jede:r Reviewer:in bekommt einen eigenen Code.
